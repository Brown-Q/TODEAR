    ...
    with open(test_file, 'r', encoding='utf-8') as test_f:
        ...
        for line in lines:
            ...
            if s_ not in entity_dict:
                entity_dict[s_] = np.random.uniform(-6 / math.sqrt(100), 6 / math.sqrt(100), 100)
            r_ = quadruple[1]
            if r_ not in relation_dict:
                relation_dict[r_] = np.random.uniform(-6 / math.sqrt(100), 6 / math.sqrt(100), 100)
            o_ = quadruple[2]
            if o_ not in entity_dict:
                entity_dict[o_] = np.random.uniform(-6 / math.sqrt(100), 6 / math.sqrt(100), 100)
            t_ = quadruple[3]
            if t_ not in time_dict:
                time_dict[t_] = np.random.uniform(-6 / math.sqrt(100), 6 / math.sqrt(100), 100)
