...
class PolicyMLP(nn.Module):
    ...
    def forward(self, state_query):
        hidden = self.mlp_l1(state_query)
        output = self.mlp_l2(hidden).unsqueeze(1)
        return output

class RewardRatio(nn.Module):
    def __init__(self, config):
        super(RewardRatio, self).__init__()
        self.mlp_l1= nn.Linear(config['max_action_num'], config['mlp_hidden_dim'], bias=True)
        self.mlp_l2 = nn.Linear(config['mlp_hidden_dim'], config['max_action_num'], bias=True)

    def forward(self, logits):
        hidden = torch.relu(self.mlp_l1(logits))
        output = self.mlp_l2(hidden)
        return output

class DynamicEmbedding(nn.Module):
    def __init__(self, n_ent, dim_ent, dim_t, data_path, time_span):
        ...
        self.ent_embs = torch.from_numpy(embeddings).float().cuda()
        # print(embeddings)
...
class Agent(nn.Module):
    def __init__(self, config):
        ...
        if self.config['entities_embeds_method'] == 'dynamic':
            self.ent_embs = DynamicEmbedding(config['num_ent']+1, config['ent_dim'], config['time_dim'], config['data_path'], config['time_span'])
        else:
            self.ent_embs = StaticEmbedding(config['num_ent']+1, config['ent_dim'])
        self.file = config['data_path']
        embeddings = np.empty(shape=[0, 100])       
        with open(self.file+"/res/relation_emb.txt", 'r', encoding='utf-8') as e_f:
            lines = e_f.readlines()
            for line in lines:
    ...
    def forward(self, prev_relation, current_entities, current_timestamps,
        ...
        output = self.policy_mlp(agent_state)  # [batch_size, 1, action_dim] action_dim == rel_dim + ent_dim  state
        ...
        actions = torch.cat([neighbors_relations, neighbors_entities], dim=-1)  # [batch_size, action_number, action_dim] all optional action
        ...
        # action_id = torch.argmax(action_prob, dim=1).unsqueeze(-1)
    ...
    def get_im_embedding(self, cooccurrence_entities):
        ...
        entities = self.ent_embs.ent_embs[cooccurrence_entities]
    ...
    def update_entity_embedding(self, entity, ims, mu):
        ...
        self.source_entity = self.ent_embs.ent_embs[entity]
        self.ent_embs.ent_embs[entity] = mu * self.source_entity + (1 - mu) * torch.mean(ims, dim=0)
    ...
    def entities_embedding_shift(self, entity, im, mu):
        ...
        self.ent_embs.ent_embs[entity] = mu * self.source_entity + (1 - mu) * im
    def back_entities_embedding(self, entity):
        ...
        self.ent_embs.ent_embs[entity] = self.source_entity
